...
async function shot(vp,theme,name,steps,url){
  ...
  await page.route('**/*',async route=>{ const u=route.request().url();
    if(u==='https://grup-al.com/'||u.startsWith('https://grup-al.com/juri')||u.startsWith('https://grup-al.com/?')) return route.fulfill({status:200,contentType:'text/html; charset=utf-8',body:html});
  ...
  await page.goto(url||'https://grup-al.com/juri'); await page.waitForTimeout(1200);
...
await shot({width:390,height:844},'red','phone_root_red', null, 'https://grup-al.com/');
await browser.close();
